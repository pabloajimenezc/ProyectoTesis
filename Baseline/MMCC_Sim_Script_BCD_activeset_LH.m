...
Wg=2*pi*Fg;
Wm=2*pi*Fm;
Tf=1; %Simulation time, seconds
Ts=2e-5;	%Sample time
Ns=round(Tf/Ts+1); %includes 0, number of simulation steps
Tsim=linspace(0,Tf,Ns);
Ti=1e-4; %Current Control MPC Sample Time
Tl=2e-4; %Energy Control MPC Superstep
Ni=round(Tf/Ti+1);
Nl=round(Tf/Tl+1);
Np=10; %internal
Tss=Tl/Np; %internal control subtime step
Tp=Tss*Np;
    ...
    if any(Tcur==Tsim(t)) %control interrupt
        ...
        if any(Ten==Tsim(t))
            ...
            for l=1:lmax
                ...
                Y=zeros(6,Np);
                for k=1:Np
                   Dn((9*k-8):9*Np,(9*k-8):9*k)=repmat(vonpred*eye(9),Np+1-k,1);
                end
                Dt=Dn+DV;
                for k=1:Np
                    D0((9*k-8):9*k,:)=diag(sum(Dt((9*k-8):9*k,:),2));
                end
                Ecprom=mean(Ec(:,t))*ones(m,1);
                H=2*((Tss)^2*transpose(D0*N)*(D0*N)+lambdao*(N')*N); 
                dz=repmat(Ec(:,t),Np,1)-repmat(Ecprom,Np,1)+Ti*Dt*reshape(ixypred,[9*Np,1]);
                f=2*Tss*transpose(D0*N)*dz;
                ...
                for k=1:Np
                    if k==1
                        DI((9*k-8):9*k,1)=ixypred(:,k)+izp;
                    else
                        DI((9*k-8):9*k,1)=ixypred(:,k)+izp+DI((9*(k-1)-8):9*(k-1),1);                
                    end
                end
                y=repmat(Ec(:,t),Np,1)+Tss*DV*reshape(ixypred+izp,[9*Np,1])-repmat(Ecprom,Np,1);
                H=2*((Tss)^2*transpose(DI)*DI+lambdav*eye(1));
                f=2*Tss*transpose(DI)*y;
                %x0 = zeros(Np,1);
                ...
                options.ConstraintTolerance = 1.0e-3;
                iA0v=iAv;
                [ZX,exitflag,iAv,muv]=mpcActiveSetSolver(H,f,[1;-1],[ub;-lb],zeros(0,length(f)),zeros(0,1),iA0v,options); 
                vonpred=ZX;     
            end     
            Tcec(t)=toc;
            iz(:,t)=izp;
