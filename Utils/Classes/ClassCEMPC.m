...
methods
    function obj = ClassCEMPC(specs)
        ...
        obj.lmax = specs.lmax;
        obj.Np = specs.Np;
        obj.NN = repmat({obj.M3C.N}, obj.Np, 1);
    ...
    function obj = control(obj, Ec, vB_pred, iB_pred)
        ...
        for l = 1:obj.lmax
            ...
            % Initialization
            vs_temp = reshape(vB_pred + von_temp', obj.M3C.m * obj.Np, 1);
            ...
            Hi_ie = eye(obj.M3C.m * obj.Np);
            fi_ie = zeros(obj.M3C.m * obj.Np, 1);
            ...
            % Solve
            ub = obj.is_max * ones(obj.M3C.m * obj.Np, 1) - iB_pred;
            lb = -obj.is_max * ones(obj.M3C.m * obj.Np, 1) - iB_pred;
            [ie_ref_temp, obj.exitflag_i, obj.iAi, ~] = mpcActiveSetSolver(H_ie, f_ie, [obj.NN; -obj.NN], [ub; -lb], zeros(0, obj.M3C.n * obj.Np), zeros(0, 1), obj.iAi, obj.options_i);
            ...
            fE_von = 2 * obj.Ts * diag(is_temp)*(e_Ec_pred + obj.Ts * diag(is_temp)*reshape(vB_pred, obj.M3C.m * obj.Np, 1));            

            % Common mode voltage penalization
            Hv_von = eye(obj.M3C.m * obj.Np);
            fv_von = zeros(obj.M3C.m * obj.Np, 1);

            % Complete weighted objective function
            H_von = HE_von + obj.lambda_o * Hv_von;
            f_von = fE_von + obj.lambda_o * fv_von;
            %
            H_von_temp = mat2cell(H_von, repmat(obj.M3C.m, 1, obj.Np), repmat(obj.M3C.m, 1, obj.Np));
            H_von_temp = arrayfun(@(i) H_von_temp{i, i}, 1:obj.Np, 'UniformOutput', false);
            H_von_temp  = cellfun(@(M) ones(1, obj.M3C.m) * M * ones(obj.M3C.m, 1), H_von_temp, 'UniformOutput', false);
            H_von = blkdiag(H_von_temp{:});
            %
            f_von_temp = mat2cell(f_von, repmat(obj.M3C.m, obj.Np, 1), 1);
            f_von_temp = cellfun(@(M) ones(1, obj.M3C.m)*M, f_von_temp, 'UniformOutput', false);
        ...
        obj.ie_ref = ie_ref_temp(1:obj.M3C.n);
        obj.iz_ref = iz_ref_temp(1:obj.M3C.m);
    ...
    function obj = reset(obj)
        ...
        obj.ie_ref = zeros(obj.M3C.n, 1);
        obj.iz_ref = zeros(obj.M3C.m, 1);
        obj.vo_ref = 0;
        obj.iAi = false(size(zeros(2 * obj.M3C.m * obj.Np, 1)));
